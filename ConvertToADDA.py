...
def main(printProgress=True):
	...
	for j, inputFilename in enumerate(inputFilenames):
		...
		for i, line in enumerate(lines):
			...
			if printProgress:
				util.percent(i,numLines,25,header=header,footer=footer,percentRange=(0,80))
		...
		for i, line in enumerate(lines):
			if len(line)>0:
				#print percent to keep progress
				if printProgress:
					util.percent(i,numLines,5,header=header,footer=footer,percentRange=(80,100))
				inArr=line.split("\t")
				query=inArr[0]
				subject=inArr[1]
				identity=inArr[2]
				...
				if float(Evalue)<EvalueCutoff and repeat==False:
					...
					writeLine=str(nid1)+"\t"+str(nid2)+"\t"+str(identity)+"\t"+from1+"\t"+to1+"\t"+str(alignlen)+"\t"+from2+"\t"+to2+"\t"+str(Evalue)+"\t"+"na"+"\t"+"na"+"\n"
