from src.class_2013_02_07_hsp import parse_blast
...
"""
    for batch_index, taxa_dir in enumerate(os.listdir(blast_dir_path)):
        open("/home/wz4/pbs/%d.pbs" % batch_index, 'w').write(
            template.format(batch_index, '/home/wz4/2013_04_17_rcm_module_tree.py',
                            '/home/wz4/epd-7.3-1-rh5-x86_64/bin/python',
                            os.path.join(blast_dir_path, taxa_dir),
                            os.path.join(output_tree_dir_path,
                                         '%d_%s.tree' % (batch_index, taxa_dir.replace(' ', '_'))),
                            os.path.join(output_tree_diagram_dir_path,
                                         '%d_%s.diagram' % (batch_index, taxa_dir.replace(' ', '_')))))
